  ...
  <div class="activity-row">
    ...
    <div class="act-metrics">
      <div class="act-metric" v-if="activity.distance_meters">
        <span class="mono">{{ distanceKm }}</span>
...
const TYPE_MAP: Record<string, { icon: string; label: string; color: string }> = {
  ...
  strength_training: { icon: '🏋️', label: 'Muscu', color: 'rgba(255,107,53,0.15)' },
}

const typeData = computed(() =>
  TYPE_MAP[props.activity.activity_type] ?? { icon: '⬡', label: props.activity.activity_type ?? '—', color: 'rgba(139,146,165,0.1)' }
)
...
const distanceKm = computed(() =>
  props.activity.distance_meters ? (props.activity.distance_meters / 1000).toFixed(2) : null
)
